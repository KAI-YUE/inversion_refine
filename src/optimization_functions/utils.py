...
def get_image_transformations(image,epochs,dict_of_losses,category):
    ...
    for direction,f in [('up',translate_up),('down',translate_down),('right',translate_right),('left',translate_left)]:
        if not os.path.exists('./gallery/{:s}'.format(direction)):
            os.makedirs('./gallery/{:s}'.format(direction))

    latent_input = produce_truncated(0.5).to(device)
    dict_of_sides = {'original':None,'left':None,'right':None,'up':None,'down':None}
    dict_of_sides['original'] = run_optim_on_variable(latent_input,image,torch.optim.Adam,epochs,[dict_of_losses['lpips_alexnet'].to(device),dict_of_losses['L1']],'biggan',category,'./gallery/')[0]
    print('done original')
    for direction,f in [('up',translate_up),('down',translate_down),('right',translate_right),('left',translate_left)]:
    # for direction,f in [('up',translate_up)]:
        ground_truth_image = f(copy.copy(image))
        dict_of_sides[direction] = run_optim_on_variable(latent_input,ground_truth_image,torch.optim.Adam,epochs,[dict_of_losses['lpips_alexnet'].to(device),dict_of_losses['L1']],'biggan',category, './gallery/{:s}'.format(direction))
